...
population <- do.call(rbind, population) |> ## Use janitor row to names function
  ...
  pivot_longer(!state_name, names_to = "year", values_to = "population") |> # use pivot_longer to tidy
  mutate(year = str_replace_all(year, "POP_", "")) |> # remove POP_ from year
  mutate(year = as.numeric(year), population = as.numeric(population)) |> # parese all relevant colunns to numeric
...
regions <- fromJSON(url, simplifyVector = FALSE) |>
  ...
  mutate(
    ...
    region_name = str_replace(region_name, "New York and New Jersey, Puerto Rico, Virgin Islands", "New York and Friends")
  ) |>
  unnest(states) |>
